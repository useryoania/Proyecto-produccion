...
import {
  ...
  Zap, ArrowRight, ChevronRight, LayoutGrid, TrendingDown, Hash, History, Wallet,
...
export default function CajaTransaccionView() {
  ...
  const [tipoDocCobro, setTipoDocCobro] = useState('ETICKET');
  ...
  const [motorTipoDoc, setMotorTipoDoc] = useState('ETICKET');
  ...
  const [ventaTipoDoc, setVentaTipoDoc] = useState('FACTURA');
  const [ventaSerieDoc, setVentaSerieDoc] = useState('C');
  ...
  const [egresoTipoDoc, setEgresoTipoDoc] = useState('RECIBO');
  ...
  const cargarDatosBasicos = async () => {
    try {
      fetchRetiros();
      const [rMet, rCot, rGastos, rOps] = await Promise.allSettled([
      ...
      if (rMet.status === 'fulfilled') {
        ...
        setMetodosPago(mets);
        // Pre-seleccionar Contado por defecto en la pestaña Venta
        const contadoId = mets.find(m => /contado/i.test(m.MPaDescripcionMetodo))?.MPaIdMetodoPago || mets[0]?.MPaIdMetodoPago;
  ...
  useEffect(() => { if (activeTab === 'CIERRE' && sesion) cargarResumenCierre(); }, [activeTab]);
      ...
      <div className="border-b border-slate-800 bg-[#0f1117] shrink-0">
        <div className="flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <button onClick={() => setShowArqueo(true)} className="bg-indigo-50 h-9 text-indigo-700 border border-indigo-200 hover:bg-indigo-600 hover:text-white px-4 rounded-xl font-black transition-all flex items-center gap-2 shadow-sm">
              <FileText size={16} /> Arqueo Turno
            </button>
            <button onClick={()=>setActiveTab('CIERRE')} className="bg-rose-50 h-9 text-rose-700 border border-rose-200 hover:bg-rose-600 hover:text-white px-4 rounded-xl font-black transition-all flex items-center gap-2 shadow-sm">
        ...
        <div className="flex gap-2 px-8 pt-4">
          {[
            ...
            { id:'CIERRE', label:'Cierre de Turno', icon:DoorClosed, color:'text-amber-600' }
          ].map(t => (
      ...
      <div className="flex-1 bg-[#f1f5f9] flex overflow-hidden">
        ...
        {activeTab === 'INGRESOS' && (
          <div className="flex-1 flex overflow-hidden">
             ...
             {/* 1) COLUMNA IZQUIERDA: MENÚ TIPO DE INGRESO */}
             <div className="w-64 bg-white border-r border-slate-200 flex flex-col pt-6 shrink-0 shadow-xl z-10">
                <div className="px-6 mb-4">
                   <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Menú de Operación</h3>
                </div>
                <div className="flex flex-col gap-2 px-3">
                   {[
                      { id:'COBRO', label:'Cobrar Órdenes', icon:ShoppingCart, desc:'Abonar retiros' },
                      { id:'VENTA', label:'Venta de Rollo por Adelantado', icon:Plus, desc:'Artículos / Servicios' },
                      { id:'SALDO_FAVOR', label:'Insertar Saldo a Favor', icon:Wallet, desc:'Servicios Varios / Ajustes' },
                      { id:'MOTOR', label:'Reglas Manuales', icon:Zap, desc:'Cobro especial, sobrantes' },
                      { id:'AUTORIZAR', label:'Autorizar Entrega', icon:ShieldCheck, desc:'Entrega sin pago (Gerencia)' }
                   ].map(st => (
                      <button key={st.id} onClick={()=>setSubTabIngreso(st.id)}
                        className={`text-left p-4 rounded-2xl flex flex-col gap-1 transition-all border-2 ${subTabIngreso === st.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200' : 'border-transparent text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}>
                        <div className="flex items-center gap-2.5 font-black text-sm"><st.icon size={18}/> {st.label}</div>
                        <span className={`text-[10px] font-bold ${subTabIngreso === st.id ? 'text-indigo-200' : 'text-slate-400'}`}>{st.desc}</span>
                      </button>
                   ))}
                </div>
             </div>
             ...
             {(subTabIngreso === 'COBRO' || subTabIngreso === 'MOTOR' || subTabIngreso === 'VENTA') && (
                <CajaPanelPago 
                   mode={subTabIngreso}
                   {...(subTabIngreso === 'COBRO' ? {
                 ...
                 {!resumenCierre ? (
                   ...
                   <div className="flex flex-col gap-10">
                     ...
                     <div className="bg-white border-2 border-indigo-100 rounded-[2.5rem] p-10 flex flex-col gap-8 shadow-xl relative">
                       ...
                       <div className="flex flex-col gap-2">
                         <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Efectivo total contado real en cajón ($)</label>
                         <input type="number" autoFocus value={cierreMontoFisico} onChange={e=>setCierreMontoFisico(e.target.value)} placeholder="0.00" className="w-full bg-slate-50/50 border-4 border-slate-100 rounded-[1.5rem] p-8 text-5xl font-black text-slate-900 outline-none focus:border-indigo-600 focus:bg-white shadow-[inset_0_4px_8px_rgba(0,0,0,0.05)] transition-all text-center tracking-tighter" />
                       </div>
