...
const getOrdenesRetiroQueryBase = `
  SELECT 
    ...
    prod.Descripcion AS orderMaterial,
  ...
  LEFT JOIN Articulos prod WITH(NOLOCK) ON prod.ProIdProducto = o.ProIdProducto
`;
  ...
  for (const row of rows) {
    if (!map[row.OReIdOrdenRetiro]) {
      map[row.OReIdOrdenRetiro] = {
        OReIdOrdenRetiro: row.OReIdOrdenRetiro,
    ...
    if (row.orderId) {
      map[row.OReIdOrdenRetiro].orders.push({
        ...
        orderMaterial:    row.orderMaterial || null,
        ...
        orderCosto:       row.costoFinal != null ? `${row.orderMonedaId === 2 ? 'US$' : (row.orderMonedaSimbolo || '$')} ${parseFloat(row.costoFinal).toFixed(2)}` : null,
        orderCantidad:    row.orderCantidad != null ? parseFloat(row.orderCantidad) : null,
        simbolo:          row.orderMonedaId === 2 ? 'US$' : (row.orderMonedaSimbolo || '$'),
...
const ordenesRetiroCaja = async (req, res) => {
  try {
    ...
    // No mostrar las que ya están "Abonadas" o cubiertas por Plan de Recursos (ORePasarPorCaja = 0)
    // NULL significa "no especificado" → se trata como pendiente de pago (igual que = 1)
    const query = `
      ${getOrdenesRetiroQueryBase}
      WHERE r.OReEstadoActual NOT IN (6)
      AND ISNULL(r.ORePasarPorCaja, 1) = 1
...
const getOrdenesRetiroPasarPorCaja = async (req, res) => {
  try {
    const pool = await getPool();
    const result = await pool.request().query(`${getOrdenesRetiroQueryBase} WHERE ISNULL(r.ORePasarPorCaja, 1) = 1`);
    res.status(200).json(processRetirosRows(result.recordset));
  } catch (err) {
...
const buscarParaMostrador = async (req, res) => {
  ...
  try {
    ...
    return res.json({ 
      retiroRows: processRetirosRows(retiroRows), 
      sinRetiro 
    });
  } catch (err) {
