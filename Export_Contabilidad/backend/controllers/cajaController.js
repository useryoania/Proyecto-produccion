...
const procesarTransaccion = async (req, res) => {
  ...
  try {
    ...
    const TIPOS = ['ETICKET','FACTURA','CREDITO','NOTA_CONSUMO','NINGUNO'];
    if (!TIPOS.includes(header.tipoDocumento))
      return res.status(400).json({ success:false, error:`tipoDocumento inválido. Opciones: ${TIPOS.join(', ')}` });
  ...
  try {
    ...
    const movimientos = await pool.request()
      ...
      SELECT 
        ...
        t.TcaFecha as Fecha,
        t.TcaTipoDocumento as TipoComprobante,
      ...
      SELECT 
        ...
        e.EgrFecha as Fecha,
        e.EgrTipoDocumento as TipoComprobante,
...
const getSiguienteNumero = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
          SecUltimoNumero + 1  AS NumeroEntero,
          ISNULL(SecPrefijo,'') +
            RIGHT(REPLICATE('0', SecDigitos) + CAST(SecUltimoNumero+1 AS VARCHAR(10)), SecDigitos)
                               AS NumeroFormato,
          SecTipoDoc AS TipoDoc,
          SecSerie   AS Serie,
          ISNULL(SecPrefijo,'') AS Prefijo
        FROM dbo.SecuenciaDocumentos WITH(NOLOCK)
        WHERE SecTipoDoc = @TipoDoc AND SecSerie = @Serie AND SecActivo = 1
      `);
    ...
    if (tipoDocumento && tipoDocumento !== 'NINGUNO' && !numeroDoc) {
      try {
        const seqR = await transaction.request()
          .input('TipoDoc', sql.VarChar(20), tipoDocumento)
          .input('Serie',   sql.VarChar(5),  serieDoc || 'A')
          .execute('SP_SiguienteNumeroDoc');
        numeroDocFinal = seqR.recordset[0]?.NumeroFormato || null;
      } catch (_) { }
    }
